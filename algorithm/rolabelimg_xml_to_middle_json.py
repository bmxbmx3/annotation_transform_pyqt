...
def rolabelimg_xml_to_middle_json(img_dir, src_label_dir, deleted_label_tuple, selected_label_tuple,
    ...
    for img_path in img_paths:
        ...
        if src_label_path in src_labels_paths:
            ...
            """转换rolabelimg rectbox xml为middle json格式"""
            bndbox_regex = "<object><type>bndbox<\/type><name>(.*?)<\/name><pose>(.*?)<\/pose><truncated>(.*?)<\/truncated><difficult>(.*?)<\/difficult><bndbox><xmin>(.*?)<\/xmin><ymin>(.*?)<\/ymin><xmax>(.*?)<\/xmax><ymax>(.*?)<\/ymax><\/bndbox><\/object>"
