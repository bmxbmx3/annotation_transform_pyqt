...
def middle_json_to_labelimg_xml(dst_label_dir):
    """
    ...
    for middle_json_path in middle_json_paths:
        ...
        for i in labels:
            ...
            if label_property == "bndbox":
                ...
                object_str = "<object><name>{}</name><pose>Unspecified</pose><truncated>0</truncated><difficult>0</difficult><bndbox><xmin>{}</xmin><ymin>{}</ymin><xmax>{}</xmax><ymax>{}</ymax></bndbox></object>"
        ...
        """拼接xml"""
        xml_out = '<annotation>' + folder_str + filename_str + path_str + source_str + size_str + \
